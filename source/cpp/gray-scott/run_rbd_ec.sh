...
# Gray-Scott on RBD-EC (RS 6,4) Runner Script
# This script provides easy commands to run Gray-Scott simulations on RBD

echo "========================================"
echo "Gray-Scott on RBD-EC (RS 6,4) Runner"
echo "RBD Mount: /mnt/rbd-ec/"
echo "========================================"

# Check if RBD is mounted
if [ ! -d "/mnt/rbd-ec" ]; then
    echo "ERROR: RBD not mounted at /mnt/rbd-ec/"
    echo "Please mount RBD first:"
    echo "sudo rbd map <pool>/<image> && sudo mount /dev/rbd0 /mnt/rbd-ec"
    exit 1
fi

# Check RBD write permissions
if [ ! -w "/mnt/rbd-ec" ]; then
    echo "ERROR: No write permissions to /mnt/rbd-ec/"
...
run_simulation() {
    ...
    local log_file="/tmp/ec_sim_performance_$(date +%Y%m%d_%H%M%S).log"
    ...
    echo "  Steps: $steps"
    echo "  Output: /mnt/rbd-ec/gray-scott/"
    echo "  Log file: $log_file"
    echo "  Start time: $(date)"
    echo ""
    
    # Create directories if they don't exist
    mkdir -p /mnt/rbd-ec/gray-scott/{checkpoints,analysis}
    ...
    local initial_usage=$(df --output=used /mnt/rbd-ec | tail -1)
    local initial_files=$(find /mnt/rbd-ec/gray-scott -type f 2>/dev/null | wc -l)
    
    echo "📊 Pre-simulation metrics:"
    echo "  RBD storage used: $(df -h /mnt/rbd-ec | tail -1 | awk '{print $3}')"
    ...
    # Start performance monitoring in background
    {
        ...
        while true; do
            ...
            local rbd_usage_mb=$(df -m /mnt/rbd-ec | tail -1 | awk '{print $3}')
    ...
    local final_usage=$(df --output=used /mnt/rbd-ec | tail -1)
    local final_files=$(find /mnt/rbd-ec/gray-scott -type f 2>/dev/null | wc -l)
    ...
    if [ $sim_exit_code -eq 0 ]; then
        ...
        echo "  Final RBD usage: $(df -h /mnt/rbd-ec | tail -1 | awk '{print $3}')"
        ...
        echo ""
        echo "📁 Output files location: /mnt/rbd-ec/gray-scott/"
        ls -lah /mnt/rbd-ec/gray-scott/
        echo ""
        echo "📊 Detailed performance log: $log_file"
        echo "   Use 'tail -f $log_file' to view real-time metrics"
        
        # Generate quick summary
        local summary_file="/tmp/ec_sim_summary_$(date +%Y%m%d_%H%M%S).txt"
        echo ""
...
run_analysis() {
    ...
    local bins=${2:-100}
    local start_time=$(date +%s.%N)
    local log_file="/tmp/ec_analysis_performance_$(date +%Y%m%d_%H%M%S).log"
    ...
    echo "  Processes: $processes"
    echo "  Bins: $bins"
    echo "  Input: /mnt/rbd-ec/gray-scott/gs-ec.bp"
    echo "  Output: /mnt/rbd-ec/gray-scott/analysis/"
    echo "  Log file: $log_file"
    echo "  Start time: $(date)"
    echo ""
    
    # Check if simulation output exists
    if [ ! -d "/mnt/rbd-ec/gray-scott/gs-ec.bp" ]; then
        echo "ERROR: No simulation output found at /mnt/rbd-ec/gray-scott/gs-ec.bp"
        echo "Please run the simulation first with: $0 sim"
        return 1
    fi
    
    # Get input file metrics
    local input_size=$(du -sm /mnt/rbd-ec/gray-scott/gs-ec.bp | cut -f1)
    local input_size_h=$(du -sh /mnt/rbd-ec/gray-scott/gs-ec.bp | cut -f1)
    local initial_usage=$(df --output=used /mnt/rbd-ec | tail -1)
    local initial_files=$(find /mnt/rbd-ec/gray-scott/analysis -type f 2>/dev/null | wc -l)
    ...
    mkdir -p /mnt/rbd-ec/gray-scott/analysis
    ...
    time mpirun --oversubscribe -n $processes ./build/adios2-pdf-calc \
        /mnt/rbd-ec/gray-scott/gs-ec.bp \
        /mnt/rbd-ec/gray-scott/analysis/pdf-ec.bp \
        $bins
    ...
    local final_usage=$(df --output=used /mnt/rbd-ec | tail -1)
    local final_files=$(find /mnt/rbd-ec/gray-scott/analysis -type f 2>/dev/null | wc -l)
    local data_written=$(echo "scale=2; ($final_usage - $initial_usage) / 1024" | bc)
    local files_created=$((final_files - initial_files))
    ...
    if [ $analysis_exit_code -eq 0 ]; then
        ...
        echo "  Memory at completion: $(free -h | grep '^Mem' | awk '{print $3}') used"
        echo ""
        echo "📁 Analysis files location: /mnt/rbd-ec/gray-scott/analysis/"
        ls -lah /mnt/rbd-ec/gray-scott/analysis/
        echo ""
        echo "📊 Detailed performance log: $log_file"
        
        # Generate analysis summary
        local summary_file="/tmp/ec_analysis_summary_$(date +%Y%m%d_%H%M%S).txt"
        echo ""
...
check_status() {
    ...
    echo "RBD Mount Point: /mnt/rbd-ec"
    echo "Data Directory: /mnt/rbd-ec/gray-scott"
    echo ""
    
    if [ -d "/mnt/rbd-ec/gray-scott" ]; then
        echo "📁 Data Directory Contents:"
        ls -la /mnt/rbd-ec/gray-scott/
        echo ""
        
        if [ -d "/mnt/rbd-ec/gray-scott/gs-ec.bp" ]; then
            echo "📊 Simulation Output:"
            echo "  Directory: gs-ec.bp"
            echo "  Size: $(du -sh /mnt/rbd-ec/gray-scott/gs-ec.bp 2>/dev/null | cut -f1 || echo 'Unknown')"
            echo "  Modified: $(stat -c %y /mnt/rbd-ec/gray-scott/gs-ec.bp 2>/dev/null || echo 'Unknown')"
        fi
        
        if [ -d "/mnt/rbd-ec/gray-scott/analysis" ]; then
            echo "📈 Analysis Directory:"
            ls -la /mnt/rbd-ec/gray-scott/analysis/
        fi
        
        if [ -d "/mnt/rbd-ec/gray-scott/checkpoints" ]; then
            echo "💾 Checkpoints Directory:"
            ls -la /mnt/rbd-ec/gray-scott/checkpoints/
        fi
...
show_performance_metrics() {
    ...
    if [ -d "/mnt/rbd-ec/gray-scott" ]; then
        ...
        echo "  Total files: $(find /mnt/rbd-ec/gray-scott -type f 2>/dev/null | wc -l)"
        echo "  Total size: $(du -sh /mnt/rbd-ec/gray-scott 2>/dev/null | cut -f1 || echo 'Unknown')"
        echo "  Simulation data: $(du -sh /mnt/rbd-ec/gray-scott/gs-ec.bp 2>/dev/null | cut -f1 || echo 'None')"
        echo "  Analysis data: $(du -sh /mnt/rbd-ec/gray-scott/analysis 2>/dev/null | cut -f1 || echo 'None')"
        echo "  Checkpoints: $(du -sh /mnt/rbd-ec/gray-scott/checkpoints 2>/dev/null | cut -f1 || echo 'None')"
    else
...
view_performance_logs() {
    ...
    case "$log_type" in
        "sim"|"simulation")
            ...
            local latest_sim_log=$(ls -t /tmp/ec_sim_performance_*.log 2>/dev/null | head -1)
        ...
        "analysis"|"analyze")
            ...
            local latest_analysis_log=$(ls -t /tmp/ec_analysis_performance_*.log 2>/dev/null | head -1)
        ...
        "summary"|"summaries")
            ...
            ls -lt /tmp/ec_sim_summary_*.txt 2>/dev/null | head -3 | while read -r line; do
            ...
            ls -lt /tmp/ec_analysis_summary_*.txt 2>/dev/null | head -3 | while read -r line; do
                local file=$(echo "$line" | awk '{print $9}')
                if [ -n "$file" ]; then
                    echo "  📄 $file"
                    echo "     $(stat -c %y "$file")"
            ...
            echo "  cat /tmp/ec_sim_summary_YYYYMMDD_HHMMSS.txt"
            echo "  cat /tmp/ec_analysis_summary_YYYYMMDD_HHMMSS.txt"
            ;;
        *)
            echo "�📊 All Performance Logs:"
            echo ""
            echo "🔬 Simulation logs:"
            ls -lt /tmp/ec_sim_performance_*.log 2>/dev/null | head -3 || echo "  None found"
            echo ""
            echo "📈 Analysis logs:"
            ls -lt /tmp/ec_analysis_performance_*.log 2>/dev/null | head -3 || echo "  None found"
            echo ""
            echo "📋 Summary files:"
...
run_full_workflow() {
    ...
    if [ $sim_result -eq 0 ]; then
        ...
        local latest_sim_summary=$(ls -t /tmp/ec_sim_summary_*.txt 2>/dev/null | head -1)
        ...
        local latest_analysis_summary=$(ls -t /tmp/ec_analysis_summary_*.txt 2>/dev/null | head -1)
...
run_performance_comparison() {
    ...
    time_ec_sim=$(date +%s)
    run_simulation $processes $steps 2>&1 | tee /tmp/performance_comparison/ec_sim.log
    time_ec_sim_end=$(date +%s)
    ec_sim_time=$((time_ec_sim_end - time_ec_sim))
    
    # Run RBD analysis
    echo "2️⃣ Running RBD analysis..."
    time_ec_analysis=$(date +%s)
    run_analysis $processes 100 2>&1 | tee /tmp/performance_comparison/ec_analysis.log
    time_ec_analysis_end=$(date +%s)
    ec_analysis_time=$((time_ec_analysis_end - time_ec_analysis))
    ...
    echo "  RBD Simulation: ${ec_sim_time}s" >> /tmp/performance_comparison/report.txt
    echo "  CephFS Simulation: ${cephfs_sim_time}s" >> /tmp/performance_comparison/report.txt
    echo "  Performance Difference: $(echo "scale=2; $cephfs_sim_time/$ec_sim_time" | bc)x" >> /tmp/performance_comparison/report.txt
    echo "" >> /tmp/performance_comparison/report.txt
    echo "Analysis Performance:" >> /tmp/performance_comparison/report.txt
    echo "  RBD Analysis: ${ec_analysis_time}s" >> /tmp/performance_comparison/report.txt
    echo "  CephFS Analysis: ${cephfs_analysis_time}s" >> /tmp/performance_comparison/report.txt
    echo "  Performance Difference: $(echo "scale=2; $cephfs_analysis_time/$ec_analysis_time" | bc)x" >> /tmp/performance_comparison/report.txt
    echo "" >> /tmp/performance_comparison/report.txt
    echo "Total Performance:" >> /tmp/performance_comparison/report.txt
    echo "  RBD Total: $((ec_sim_time + ec_analysis_time))s" >> /tmp/performance_comparison/report.txt
    echo "  CephFS Total: $((cephfs_sim_time + cephfs_analysis_time))s" >> /tmp/performance_comparison/report.txt
    echo "  Overall Difference: $(echo "scale=2; ($cephfs_sim_time + $cephfs_analysis_time)/($ec_sim_time + $ec_analysis_time)" | bc)x" >> /tmp/performance_comparison/report.txt
...
case "$1" in
    ...
    *)
        ...
        echo ""
        echo "Prerequisites:"
        echo "  - RBD must be mounted at /mnt/rbd-ec"
